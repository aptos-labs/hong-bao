module addr::dirichlet {
    ...
    public(friend) fun sequential_dirichlet_hongbao(total_amount: u64, num_packets: u64): vector<FixedPoint32> {
        ...
        let remaining = fixed_point32::create_from_u64(total_amount);

        while (!draws.is_empty()) {
            let draw = draws.pop_back();
            let amount =
                mul_div(draw, fixed_point32::create_from_u64(total_amount), running_sum);
            amounts.push_back(amount);
            remaining = fixed_point32::create_from_raw_value(
                remaining.get_raw_value() - amount.get_raw_value()
            );
        };
    ...
    public fun test_sequential_dirichlet_hongbao(aptos_framework: &signer) {
        ...
        let amounts = sequential_dirichlet_hongbao(total_amount, num_packets);

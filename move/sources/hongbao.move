module addr::hongbao {
    ...
    /// You tried to reclaim a Gift that you don't own.
    ...
    entry fun snatch_envelope(
        ...
        let amount = randomness::u64_range(0, remaining_amount + 1);
    ...
    /// also delete the object to get the gas refund.
    public entry fun reclaim_gift(caller: &signer, gift: Object<Gift>) acquires Gift {
        let caller_address = signer::address_of(caller);

        // Assert that the caller is the owner of the Gift.
        assert!(
            object::is_owner(gift, caller_address), error::invalid_state(E_NOT_OWNER)
        );
